...
double scale( double val, double min, double max, int type ) {
    if ( min == max )
        return ( val ); /* i.e. Don't Scale this data */
    ...
    for ( i = 0; i < net->numLayers; i++ ) {
        ...
        for ( j = 0; j < net->numNeurons[ i ]; j++ ) {
            ...
            for ( k = 0; k <= nTemp->numInputs; k++ ) {
                ...
                nTemp->deltaWeights[ k ] = net->learnRate * input * nTemp->delta + net->momentum * nTemp->deltaWeights[ k ];
...
void runNetworkOnce( mlpNetwork* net, dataset* data, int print ) {
    ...
    if ( print > 0 ) {
        ...
        case 2: /* Outputs and targets */
            printf( "%-.*s|%-.*s\n", ( 9 * data->numInputs ) - 2, "Inputs: ", 19 * data->numOutputs, "(Outputs,Targets): " );
            break;
        case 3: /* Outputs and errors */
            printf( "%-.*s|%-.*s\n", ( 9 * data->numInputs ) - 2, "Inputs: ", 19 * data->numOutputs, "(Outputs,Errors):  " );
            break;
        case 4: /* Outputs, target and errors */
            printf( "%-.*s|%-.*s\n", ( 9 * data->numInputs ) - 2, "Inputs: ", 28 * data->numOutputs, "(Outputs,Targets,Errors):   " );
            break;
        case 1: /* Just the outputs */
        default:
            printf( "%-.*s|%-.*s\n", ( 9 * data->numInputs ) - 2, "Inputs: ", 10 * data->numOutputs, "(Outputs):" );
            break;
        }
        printf( "\n" );
    }
    ...
    for ( i = 0; i < data->numMembers; i++ ) {
        ...
        if ( print > 0 ) {
            for ( k = 0; k < data->numInputs; k++ ) {
                ...
                min = data->minScale[ k ];
                if ( k == 0 )
                    printf( "%7.4lf", scale( member->inputs[ k ], min, max, SCALE_FOR_HUMAN ) );
                else
                    printf( ", %7.4lf", scale( member->inputs[ k ], min, max, SCALE_FOR_HUMAN ) );
            }
            printf( "|" );
            for ( k = 0; k < data->numOutputs; k++ ) {
                ...
                case 3: /* Outputs and errors */
                    ...
                    break;
                case 4: /* Outputs, target and errors */
                    printf( "(%7.4lf, %7.4lf, %7.4lf) ", scale( member->outputs[ k ], min, max, SCALE_FOR_HUMAN ), scale( member->targets[ k ], min, max, SCALE_FOR_HUMAN ),
                            scale( member->errors[ k ], min, max, SCALE_FOR_HUMAN ) );
                    break;
                case 1: /* Just the outputs */
                default:
...
mlpNetwork* createNetwork( int numLayers, int* numPerLayer, int inputs, int learnMethod, int defaultActivation ) {
    ...
    if ( learnMethod != BPROP_LEARNING ) {
        if ( learnMethod == HEBB_LEARNING )
            printf( "The hebbian learning method is not implemented yet\n" );
        else
            printf( "The learning method is not recognised\n" );
        return ( NULL );
    }
    if ( defaultActivation != LIN_ACTIVATION && defaultActivation != SIG_ACTIVATION ) {
        printf( "Activation method not recognised\n" );
        return ( NULL );
    }

    if ( ( net = ( mlpNetwork* )malloc( sizeof( mlpNetwork ) ) ) == NULL )
        return ( NULL );
    ...
    if ( ( net->layers = ( neuron** )malloc( numLayers * sizeof( neuron* ) ) ) != NULL )
        check |= 0x01;

    if ( ( net->numNeurons = ( int* )malloc( numLayers * sizeof( int ) ) ) != NULL )
        check |= 0x02;
    ...
    for ( i = 0; i < numLayers; i++ ) {
        ...
        for ( j = 0; j < numPerLayer[ i ]; j++ ) {
            ...
            if ( i == 0 )
                nTemp->numInputs = inputs; /* If it's the input layer */
            else
                nTemp->numInputs = numPerLayer[ i - 1 ]; /* If it's not the input layer */
            ...
            check = 0x00;
            if ( ( nTemp->weights = ( double* )malloc( ( nTemp->numInputs + 1 ) * sizeof( double ) ) ) != NULL )
                check |= 0x01;
            if ( ( nTemp->inputs = ( double** )malloc( ( nTemp->numInputs ) * sizeof( double* ) ) ) != NULL )
                check |= 0x02;

            /* Allocate memory for the weight change array */
            if ( ( nTemp->deltaWeights = ( double* )malloc( ( nTemp->numInputs + 1 ) * sizeof( double ) ) ) != NULL )
                check |= 0x04;
